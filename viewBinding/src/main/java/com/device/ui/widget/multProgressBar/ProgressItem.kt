...
import com.device.ui.utils.*
import com.device.ui.utils.applyCorrection
import com.device.ui.utils.getRotatedAnimator
import com.device.ui.utils.getStateList
import com.device.ui.utils.ringDuration
...
class ProgressItem : View{
    ...
    private inner class TextDrawer{
        ...
        private fun calculatePositions(){
                    ...
                    fun calculate(suggestGravity : Int) =  when {
                        ...
                        suggestGravity == TEXT_GRAVITY_IN && leftFree > requireWight -> {
                            ...
                            if( orientation == ORIENTATION_RIGHT){
                                ...
                                if (start <= 0 ) TextPosition.Position(
                                    360 + start,
                                    360 + zeroPosition
                                )
                                ...
                                if(end < 0) TextPosition.Position(
                                    zeroPosition + roundingOffset,
                                    -end
                                )
                        ...
                        suggestGravity == TEXT_GRAVITY_OUT && rightFree > requireWight -> {
                            ...
                            if (orientation == ORIENTATION_RIGHT){
                                ...
                                TextPosition.Position(
                                    360 - zeroPosition - endAng - roundingOffset,
                                    360 - zeroPosition - roundingOffset
                                )
                            }
                        }
                        else -> {
            ...
            positions.addAll(suggestPositions(
                    ...
                    showLabel = showLabel,
                    zeroPosition = 0f).mapIndexed { index, textPosition ->
                ShownText.Label(
                    textPosition,
            ...
            positions.addAll(suggestPositions(
                    ...
                    .mapIndexed { index, textPosition ->
                        ShownText.Progress(
                            textPosition,
                            (index == 0 && textPosition != TextPosition.NotShowing)
                        )
                    })
            positions.addAll(suggestPositions(
    ...
    fun setSecondaryProgress(progress : Int, animate : Boolean){
        ...
        if (!animate){
            ...
            val animator = getValueAnimator(this,
                ProgressItem::currentSecondaryProgress,currentSecondaryProgress,this.secondaryProgress.toFloat())
