...
import net.sgztech.timeboat.databinding.FragmentBloodOxDayBinding
...
class BloodOxDayFragment :BaseFragment() {

    private val bloodOxViewModel : BloodOxViewModel by viewModels()
    private val dayBinding : FragmentBloodOxDayBinding by viewBinding()
    ...
    companion object{
        fun newInstance(label: String ): BloodOxDayFragment {
            val fragment = BloodOxDayFragment()
        ...
        bloodOxViewModel.bloodOxData.vmObserver(this){
            ...
            onAppSuccess = {
                if(it!=null){
                    ...
                    dayBinding.averageBloodOxCount.text="" + it.info.averageBloodOxygen
//                    setData()
    ...
    private fun initChar(){
        ...
        //没有数据时显示的文字
        dayChat.setNoDataText("没有血氧数据");//没有数据时显示的文字
        //没有数据时显示文字的颜色
...
//        yAxisLine.axisMaximum = 200f
    ...
    private fun getSportEntry(sourceData :List<BloodOxygenModel>): List<Entry>{
        val values = ArrayList<Entry>()
        for (item  in sourceData){
            val id =  changeDateToId(item.deviceTime)
            values.add(Entry(id.toFloat(), item.bloodOxygen.toFloat()))
