...
class BleDisconnectedExceptionTest extends Specification {
    ...
    def "toString should include message with unknown status"() {
        ...
        assert objectUnderTest.toString() ==
                "com.device.rxandroidble2.exceptions.BleDisconnectedException: Disconnected from MAC='XX:XX:XX:XX:XX:XX' with status -1 (UNKNOWN)"
    }
    ...
    def "toString should include message with status"() {
        ...
        then:
        assert objectUnderTest.toString() ==
                "com.device.rxandroidble2.exceptions.BleDisconnectedException: Disconnected from MAC='XX:XX:XX:XX:XX:XX' with status $expectedStatus (GATT_INTERNAL_ERROR)"
    }
}
